<template>
  <div class="toOtherTran">
    <div class="toOtherTran_box " :class="{myActive : showtemToOtherBol}">
      <div class="toOtherTran_box_center " :class="{myActive:NicknameShow}" id="captcha">
        <p class="off_icon">
...
<script>
  ...
  export default {
    name: "toOtherTran",
    props: ['showtemToOtherBol', 'initTempMoney'],
    data() {
      return {
        passwad: '',
        userName: '',
      ...
      initNECaptcha(
        {
          ...
          onVerify(err, data) {
            if (!err) {
              ...
              getdata('/Supernode/tempTransferOther', {
                token: self.$token,
                mark: self.$mark,
                ...
                if (res.data.code == 10000) {
                  ...
                  self.$emit('hideToOtherTran', true);
                  self.userName = '';
                  self.passwad = '';
                  self.moneyNum = '';
                  self.other_id = '';
    ...
    methods: {
      ...
      validateMoneyNum() {
        ...
        if (isNaN(money)) {
          ...
          if (money != '') {
            if (Number(money) > Number(this.initTempMoney)) {
              this.moneyNum = money.substr(0, money.length - 1);
